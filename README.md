# Sentri — Real-time Fraud Detection Engine

> Kafka streaming pipeline · XGBoost + Isolation Forest · FastAPI · React dashboard

A production-grade fraud detection system that streams transactions through Apache Kafka, scores each one using an ML ensemble in real-time, and displays live results on a React dashboard.

## Flow

Producer → Kafka → ML Scoring Engine → SQLite + FastAPI → React Dashboard
...
Download creditcard.csv from Kaggle (kaggle.com/datasets/mlg-ulb/creditcardfraud) and place it in ml/data/

### 3. Train models

Open ml/notebooks/sentri_model_training.ipynb in Jupyter and run all cells. Saves models to ml/models/

### 4. Start Kafka

Download Apache Kafka 3.7.0, extract to C:\kafka then run:
...
Built by Vinay (github.com/vinaydev00)
